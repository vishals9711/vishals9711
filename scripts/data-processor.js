...
class DataProcessor {
  ...
  processGithubData(githubStats) {
    ...
    if (githubStats.recent_repos && githubStats.recent_repos.length > 0) {
      const repoLines = githubStats.recent_repos.map(repo => {
        const stars = this.formatNumber(repo.stars);
        return `- **[${repo.name}](${repo.url})** - ${repo.description} (${repo.primary_language}) ⭐ ${stars}`;
      });
  ...
  async generateDynamicAboutSection(githubStats, wakatimeStats) {
    if (this.llmClient && process.env.LLM_API_KEY) {
      const prompt = `Given the following GitHub statistics: total contributions ${githubStats.total_contributions}, total pull requests ${githubStats.total_prs}, total issues ${githubStats.total_issues}, and Wakatime statistics: total coding seconds ${wakatimeStats.total_seconds}, top language ${wakatimeStats.languages[0]?.name || 'Unknown'}, generate a concise 'About Me' section for a GitHub profile README. Start with "Software Engineer @ TNM".`;
      try {
  ...
  async generateFunFacts(githubStats, wakatimeStats) {
    if (this.llmClient && process.env.LLM_API_KEY) {
      const prompt = `Based on these GitHub stats: total contributions ${githubStats.total_contributions}, total pull requests ${githubStats.total_prs}, total issues ${githubStats.total_issues}, and Wakatime stats: total coding seconds ${wakatimeStats.total_seconds}, list 3-4 fun facts about my coding activity for a GitHub README. Start each fact with an emoji.`;
      try {
        const llmResponse = await this.llmClient.generateText(prompt);
        return llmResponse;
      } catch (error) {
  ...
  async generateMotivationalQuote(githubStats, wakatimeStats) {
    if (this.llmClient && process.env.LLM_API_KEY) {
      const prompt = `Generate a single short motivational quote about coding, suitable for a GitHub profile README. Consider that the user's total GitHub contributions are ${githubStats.total_contributions} and Wakatime total coding seconds are ${wakatimeStats.total_seconds}.`;
      try {
        const llmResponse = await this.llmClient.generateText(prompt);
        return llmResponse;
      } catch (error) {
